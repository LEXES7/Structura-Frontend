...
export default function AdminLanding() {
  ...
  const generateReport = async () => {
    ...
    try {
      ...
      // Add report metadata - with increased spacing
      const metadataStartY = 40; // Increased starting position
      pdf.setFontSize(12);
      pdf.text(`Generated on ${format(new Date(), 'PPpp')}`, 20, metadataStartY);
      pdf.text(`Generated by: ${currentUser.username}`, 20, metadataStartY + 7);
      pdf.text(`Report period: All time statistics`, 20, metadataStartY + 14);

      // Add section divider
      pdf.setDrawColor(200, 200, 200);
      pdf.line(20, metadataStartY + 20, 190, metadataStartY + 20);

      // Add summary section - with increased spacing
      const summaryStartY = metadataStartY + 30; // Added more spacing
      pdf.setFontSize(16);
      pdf.setTextColor(59, 130, 246);
      pdf.text('Platform Summary', 20, summaryStartY);
      
      pdf.setTextColor(0, 0, 0);
      pdf.setFontSize(10);
      pdf.text('This report provides an overview of key platform metrics and user activity.', 20, summaryStartY + 7);

      // Key metrics table - with increased spacing
      const metricsStartY = summaryStartY + 20; // Added more spacing
      pdf.setFontSize(14);
      pdf.setTextColor(59, 130, 246);
      pdf.text('Key Metrics', 105, metricsStartY, { align: 'center' });
      ...
      // Use autoTable with increased spacing
      autoTable(pdf, {
      ...
      // Add section divider
      let finalY = pdf.previousAutoTable ? pdf.previousAutoTable.finalY : metricsStartY + 50;
      pdf.setDrawColor(200, 200, 200);
      pdf.line(20, finalY + 10, 190, finalY + 10);

      // Reviews by rating table - with increased spacing
      pdf.setFontSize(14);
      pdf.setTextColor(59, 130, 246);
      pdf.text('Reviews by Rating', 105, finalY + 25, { align: 'center' });
      ...
      // Use autoTable directly with increased spacing
      autoTable(pdf, {
        startY: finalY + 30,
      ...
      pdf.setFillColor(59, 130, 246); // Blue header
      pdf.rect(0, 0, 210, 20, 'F'); // Slightly taller header
      pdf.setTextColor(255, 255, 255);
      pdf.setFontSize(16);
      pdf.text('VISUALIZED DATA', 105, 13, { align: 'center' });
      
      pdf.setTextColor(0, 0, 0);
      
      // Add section intro
      pdf.setFontSize(10);
      pdf.text('The following charts provide visual representation of key platform metrics.', 20, 30);
      ...
      // Add the charts to the PDF - with increased spacing between charts
      let yPosition = 40; // Start charts lower on the page
      
      // User Growth Chart
      if (userChartImg) {
        pdf.setFontSize(14);
        pdf.setTextColor(59, 130, 246);
        pdf.text('User Growth (Last 6 Months)', 105, yPosition, { align: 'center' });
        
        // Add description
        pdf.setTextColor(0, 0, 0);
        pdf.setFontSize(9);
        pdf.text('Monthly trend of new user registrations over the past 6 months', 105, yPosition + 7, { align: 'center' });
        
        const imgWidth = 170;
        const imgHeight = 70;
        
        pdf.addImage(userChartImg, 'PNG', (pageWidth - imgWidth) / 2, yPosition + 15, imgWidth, imgHeight);
        yPosition += imgHeight + 35; // Increased spacing between charts
      }
      ...
      if (postChartImg) {
        ...
        pdf.line(50, yPosition - 10, 160, yPosition - 10);
        
        pdf.setFontSize(14);
        pdf.setTextColor(59, 130, 246);
        pdf.text('Posts by Category', 105, yPosition, { align: 'center' });
        
        // Add description
        pdf.setTextColor(0, 0, 0);
        pdf.setFontSize(9);
        pdf.text('Distribution of posts across different content categories', 105, yPosition + 7, { align: 'center' });
        
        const imgWidth = 170;
        const imgHeight = 70;
        
        pdf.addImage(postChartImg, 'PNG', (pageWidth - imgWidth) / 2, yPosition + 15, imgWidth, imgHeight);
        yPosition += imgHeight + 35; // Increased spacing between charts
      }
      ...
      pdf.setTextColor(255, 255, 255);
      pdf.setFontSize(16);
      pdf.text('VISUALIZED DATA (CONTINUED)', 105, 13, { align: 'center' });
      
      pdf.setTextColor(0, 0, 0);
      yPosition = 40; // Start charts lower on the page
      
      // Review Rating Chart
      if (reviewChartImg) {
        pdf.setFontSize(14);
        pdf.setTextColor(59, 130, 246);
        pdf.text('Review Ratings Distribution', 105, yPosition, { align: 'center' });
        
        // Add description
        pdf.setTextColor(0, 0, 0);
        pdf.setFontSize(9);
        pdf.text('Distribution of review ratings from 1 to 5 stars', 105, yPosition + 7, { align: 'center' });
        
        const imgWidth = 140;
        const imgHeight = 90;
        
        pdf.addImage(reviewChartImg, 'PNG', (pageWidth - imgWidth) / 2, yPosition + 15, imgWidth, imgHeight);
        yPosition += imgHeight + 35; // Increased spacing between charts
      }
      ...
      if (courseChartImg) {
        ...
        pdf.setFontSize(14);
        pdf.setTextColor(59, 130, 246);
        pdf.text('Courses by Category', 105, yPosition, { align: 'center' });
        
        // Add description
        pdf.setTextColor(0, 0, 0);
        pdf.setFontSize(9);
        pdf.text('Distribution of courses across different educational categories', 105, yPosition + 7, { align: 'center' });
        
        const imgWidth = 170;
        const imgHeight = 80;
        
        pdf.addImage(courseChartImg, 'PNG', (pageWidth - imgWidth) / 2, yPosition + 15, imgWidth, imgHeight);
      }
      ...
      pdf.setTextColor(255, 255, 255);
      pdf.setFontSize(16);
      pdf.text('REVIEW ANALYSIS', 105, 13, { align: 'center' });
      
      // Add section intro with increased spacing
      pdf.setTextColor(0, 0, 0);
      pdf.setFontSize(10);
      pdf.text('Detailed analysis of user reviews and ratings across the platform.', 20, 30);
      
      // Review summary with better spacing
      const reviewSummaryY = 45;
      pdf.setFontSize(14);
      pdf.setTextColor(59, 130, 246);
      pdf.text('Summary', 20, reviewSummaryY);
      
      pdf.setTextColor(0, 0, 0);
      pdf.setFontSize(11);
      pdf.text(`Total Reviews: ${stats.reviewCount}`, 20, reviewSummaryY + 10);
      pdf.text(`Average Rating: ${stats.averageRating} / 5`, 20, reviewSummaryY + 18);
      ...
      if (stats.reviewCount > 0) {
        ...
        // Draw sentiment bars with color
        const barWidth = 100;
        const barHeight = 6;
        const barStartX = 130;
        const barTextX = barStartX + barWidth + 5;
        
        // Positive bar
        pdf.text(`Positive Reviews (4-5 stars):`, 20, reviewSummaryY + 30);
        pdf.setFillColor(75, 192, 192); // Green-blue
        pdf.rect(barStartX, reviewSummaryY + 27, barWidth * (positiveCount / stats.reviewCount), barHeight, 'F');
        pdf.text(`${positiveCount} (${positivePercentage}%)`, barTextX, reviewSummaryY + 30);
        
        // Neutral bar
        pdf.text(`Neutral Reviews (3 stars):`, 20, reviewSummaryY + 42);
        pdf.setFillColor(255, 205, 86); // Yellow
        pdf.rect(barStartX, reviewSummaryY + 39, barWidth * (neutralCount / stats.reviewCount), barHeight, 'F');
        pdf.text(`${neutralCount} (${neutralPercentage}%)`, barTextX, reviewSummaryY + 42);
        
        // Negative bar
        pdf.text(`Needs Improvement (1-2 stars):`, 20, reviewSummaryY + 54);
        pdf.setFillColor(255, 99, 132); // Red
        pdf.rect(barStartX, reviewSummaryY + 51, barWidth * (negativeCount / stats.reviewCount), barHeight, 'F');
        pdf.text(`${negativeCount} (${negativePercentage}%)`, barTextX, reviewSummaryY + 54);
      }
      
      // Add section divider
      pdf.setDrawColor(200, 200, 200);
      pdf.line(20, reviewSummaryY + 65, 190, reviewSummaryY + 65);
      
      // Review details table with more spacing
      pdf.setFontSize(14);
      pdf.setTextColor(59, 130, 246);
      pdf.text('Detailed Rating Distribution', 105, reviewSummaryY + 80, { align: 'center' });
      ...
      // Use autoTable directly with better spacing
      autoTable(pdf, {
        startY: reviewSummaryY + 85,
        ...
        margin: { top: 15 } // Add margin for better spacing
      });
      
      // Recommendations based on ratings with increased spacing
      finalY = pdf.previousAutoTable ? pdf.previousAutoTable.finalY : reviewSummaryY + 150;
      ...
      pdf.setFontSize(14);
      pdf.setTextColor(59, 130, 246);
      pdf.text('Recommendations', 105, finalY + 25, { align: 'center' });
      
      // Add recommendation icon based on rating
      if (stats.averageRating >= 4.5) {
        pdf.setTextColor(75, 192, 192);
        pdf.setFontSize(16);
        pdf.text('★★★★★', 105, finalY + 35, { align: 'center' });
      ...
      const recommendationY = finalY + 45; // Increased spacing
      ...
      pdf.setFontSize(14);
      pdf.setTextColor(59, 130, 246);
      pdf.text('Conclusion', 105, recommendationY + 25, { align: 'center' });
      
      pdf.setTextColor(0, 0, 0);
      pdf.setFontSize(10);
      const conclusionText = `This report provides a snapshot of platform performance as of ${format(new Date(), 'MMMM d, yyyy')}. Regular monitoring of these metrics is recommended to track growth and identify areas for improvement.`;
      const splitConclusion = pdf.splitTextToSize(conclusionText, pageWidth - 40);
      pdf.text(splitConclusion, 20, recommendationY + 35);
      ...
      for (let i = 1; i <= pageCount; i++) {
        ...
        pdf.roundedRect(5, 5, 200, 287, 2, 2, 'S');
        
        pdf.setFontSize(8);
        pdf.setTextColor(100, 100, 100);
        pdf.text(`Report generated from Structura Admin Dashboard - Page ${i} of ${pageCount}`, 105, 285, { align: 'center' });
        pdf.text(`© ${new Date().getFullYear()} Structura. All rights reserved.`, 105, 290, { align: 'center' });
      }
